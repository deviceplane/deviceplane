...
import { Textarea, Button, TextInput, TextInputField, Pane, Tablist, SidebarTab, Tab,
  Table, Heading, Icon, Text, Code, Card, Label, Dialog, BackButton,
  IconButton, Badge, majorScale, minorScale, toaster, Link, Checkbox,
...
class Iam extends Component {
      ...
      <Tablist border="default">
        {this.state.tabs.map((tab, index) => (
          <Tab
            key={tab}
            id={tab}
            onSelect={() => this.props.history.push(`/${projectName}/iam/${tab}`)}
  ...
  renderInner = (match) => {
    ...
    switch (match.params.iamTab) {
      case 'members':
        return <MembersRouter user={user} projectName={projectName} match={match} history={this.props.history}/>
      case 'serviceaccounts':
        return <ServiceAccountsRouter projectName={projectName} match={match} history={this.props.history}/>
      case 'roles':
        return <RolesRouter projectName={projectName} match={match} history={this.props.history}/>
      default:
        return <Pane></Pane>
    }
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <TopHeader user={this.props.user} heading={heading} history={this.props.history} />
        <Pane
          display="flex"
    ...
    return (
      <Switch>
        ...
        <Route
          ...
          render={(route) => (
            <Member user={user} projectName={projectName} userId={route.match.params.userId} history={route.history} />
          )} />
        <Route
          exact
          path={match.path}
          render={(route) => (
            <Members projectName={projectName} match={match} history={route.history} />
          )} />
      </Switch>
    );
  }
        ...
        { members ? (
          <InnerCard>
            ...
            >
              ...
              <Button margin={majorScale(2)} appearance="primary" onClick={() => this.props.history.push(`/${this.props.projectName}/iam/members/add`)}>
                Add Member
              </Button>
            </Pane>
            {members && members.length > 0 && (
              <Table>
                ...
                <Table.Body>
                  {members.map(member => (
                    <Table.Row key={member.userId} isSelectable onSelect={() => this.props.history.push(`/${this.props.projectName}/iam/members/${member.userId}`)}>
                      <Table.TextCell>{member.user.email}</Table.TextCell>
                      <Table.TextCell>{`${member.user.firstName} ${member.user.lastName}`}</Table.TextCell>
                      <Table.TextCell>{member.roles.map(role => role.name).join(',')}</Table.TextCell>
                    </Table.Row>
                  ))}
                </Table.Body>
              </Table>
...
class Member extends Component {
  ...
  componentDidMount() {
    ...
    .then((response) => {
      this.setState({
        ...
        roleBindings: this.createRoleBindings(response.data, this.state.allRoles)
      });
    })
    .catch((error) => {
      console.log(error);
    ...
    .then((response) => {
      this.setState({
        ...
        roleBindings: this.createRoleBindings(this.state.member, response.data)
      });
    })
    .catch((error) => {
      console.log(error);
  ...
  createRoleBindings = (member, allRoles) => {
    ...
    if (member !== null) {
      for (var i=0; i < allRoles.length; i++) {
        ...
        roleBindings.push({ id: allRoles[i].id, name: allRoles[i].name, hasRoleBinding: hasRole });
      }
    }
    return roleBindings
  }
  ...
  handleUpdateRoles = (event) => {
    ...
    for (var i = 0; i < currentRoleBindings.length; i++){
      ...
      newRoleBindings.push({ id: currentRoleBindings[i].id, name: currentRoleBindings[i].name, hasRoleBinding: hasRole });
    }
    this.setState({
        ...
        {member ? (
          <InnerCard>
            <Pane padding={majorScale(4)}>
              <Heading  size={600}>{`${member.user.firstName} ${member.user.lastName}`}</Heading>
              <Text size={400}>{member.user.email}</Text>
              <Heading size={500} paddingTop={majorScale(4)}>Choose Individual Roles</Heading>
              {roleBindings.map(role => (
                <Checkbox key={role.id} id={role.id} label={role.name} checked={role.hasRoleBinding} onChange={(event) => this.handleUpdateRoles(event)}/>
              ))}
              <Button marginTop={majorScale(2)} appearance="primary" disabled={this.state.unchanged} onClick={() => this.handleUpdate()}>Update Member</Button>
            </Pane>
            <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
              <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" disabled={this.state.isSelf} onClick={() => this.setState({ showRemoveDialog: true })}>Remove Member...</Button>
            </Pane>
            <Pane>
              <Dialog
              ...
              >
                You are about to remove the member (<strong>{member.user.firstName} {member.user.lastName}</strong>) from the project.
              </Dialog>
            </Pane>
          </InnerCard>
        ) : (
...
class AddMember extends Component {
  ...
  createRoleBindings = (allRoles) => {
    var roleBindings = [];
    for (var i = 0; i < allRoles.length; i++) {
      roleBindings.push({ id: allRoles[i].id, name: allRoles[i].name, hasRoleBinding: false });
    }
    return roleBindings
  }
  ...
  handleAddRoleBindings = (event) => {
    ...
    for (var i = 0; i < roleBindings.length; i++) {
      ...
      updatedRoleBindings.push({ id: roleBindings[i].id, name: roleBindings[i].name, hasRoleBinding: hasRole })
    }
    this.setState({
  ...
  handleSubmit = () => {
    ...
    .then((response) => {
      ...
      if (noError) {
        ...
        toaster.warning('Member was added successfully, but role bindings for the member were not updated properly. Please check the roles of the member.');
      }
    })
    .catch((error) => {
      ...
      <Pane width="50%">
        <InnerCard>
          ...
          >
            ...
            <Heading paddingBottom={majorScale(4)} size={600}>Add Member</Heading>
            <TextInputField
              label="Email"
            ...
            {roleBindings.map(role => (
              <Checkbox key={role.id} id={role.id} label={role.name} checked={role.hasRoleBinding} onChange={(event) => this.handleAddRoleBindings(event)} />
            ))}
            <Pane display="flex" flex="row">
              <Button marginTop={majorScale(2)} appearance="primary" onClick={() => this.handleSubmit()}>Add Member</Button>
    ...
    return (
      <Switch>
        <Route
          ...
          render={(route) => (
            <CreateServiceAccount projectName={projectName} history={this.props.history} />
          )} />
        <Route
          path={`${match.path}/:serviceAccountName`}
          render={(route) => (
            <ServiceAccount projectName={projectName} serviceAccountName={route.match.params.serviceAccountName} history={this.props.history} />
          )}/>
        <Route
          exact
          path={match.path}
          render={(route) => (
            <ServiceAccounts projectName={projectName} history={this.props.history} />
        )}/>
      </Switch>
    );
  }
        ...
        { serviceAccounts ? (
          <InnerCard>
            ...
            >
              ...
              <Button margin={majorScale(2)} appearance="primary" onClick={() => this.props.history.push(`/${this.props.projectName}/iam/serviceaccounts/create`)}>
                Create Service Account
              </Button>
            </Pane>
            {this.state.serviceAccounts && this.state.serviceAccounts.length > 0 && (
              <Table>
                <Table.Head>
                ...
                <Table.Body>
                  {serviceAccounts.map(serviceAccount => (
                    <Table.Row key={serviceAccount.id} isSelectable onSelect={() => this.props.history.push(`/${this.props.projectName}/iam/serviceaccounts/${serviceAccount.name}`)}>
...
class ServiceAccount extends Component {
  ...
  componentDidMount() {
    ...
    .then((response) => {
      this.setState({
        allRoles: response.data,
        roleBindings: this.createRoleBindings(this.state.serviceAccount, response.data)
      });
    })
    .catch((error) => {
      console.log(error);
    ...
    return (
      <Pane width="70%">
        { serviceAccount ? (
          <InnerCard>
            <Pane padding={majorScale(4)}>
              <Heading paddingBottom={majorScale(2)} size={600}>Service Account / {serviceAccount.name}</Heading>
              <TextInputField
                label="Name"
              ...
              />
              <Heading size={500} paddingTop={majorScale(2)}>Choose Individual Roles</Heading>
              {roleBindings.map(role => (
                <Checkbox key={role.id} id={role.id} label={role.name} checked={role.hasRoleBinding} onChange={(event) => this.handleUpdateRoles(event)} />
              ))}
              <Button marginTop={majorScale(2)} appearance="primary" disabled={this.state.unchanged} onClick={() => this.handleUpdate()}>Update Service Account</Button>
            </Pane>
            <ServiceAccountAccessKeys projectName={this.props.projectName} serviceAccount={serviceAccount} history={this.props.history}/>
            <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
              <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" onClick={() => this.setState({ showDeleteDialog: true })}>Delete Service Account...</Button>
            </Pane>
            <Pane>
              <Dialog
              ...
              >
                You are about to delete the <strong>{this.props.serviceAccountName}</strong> service account.
              </Dialog>
            </Pane>
          </InnerCard>
        ) : (
      ...
      <React.Fragment>
        ...
        >
          ...
          >
            ...
            <Button appearance="primary" onClick={() => this.createAccessKey()}>Create Access Key</Button>
          </Pane>
          {this.state.backendError && (
            <Alert marginBottom={majorScale(2)} paddingTop={majorScale(2)} paddingBottom={majorScale(2)} intent="warning" title={this.state.backendError} />
          )}
      ...
      <Pane width="50%">
        <InnerCard>
          ...
          >
            ...
            <Heading paddingBottom={majorScale(4)} size={600}>Create Service Account</Heading>
            <TextInputField
              label="Name"
              onChange={this.handleUpdateName}
    ...
    return (
      <Switch>
        <Route
          path={`${match.path}/create`}
          render={(route) => (
            <CreateRole projectName={projectName} history={this.props.history} />
        )}/>
        <Route
          path={`${match.path}/:roleName`}
          render={(route) => (
            <Role projectName={projectName} roleName={route.match.params.roleName} history={this.props.history} />
        )}/>
        <Route
          exact
          path={match.path}
        ...
        { roles ? (
          <InnerCard>
            ...
            >
              ...
              <Button margin={majorScale(2)} appearance="primary" onClick={() => this.props.history.push(`/${this.props.projectName}/iam/roles/create`)}>
                Create Role
              </Button>
            </Pane>
            {this.state.roles && this.state.roles.length > 0 && (
              <Table>
                ...
                <Table.Body>
                  {roles.map(role => (
                    <Table.Row key={role.id} isSelectable onSelect={() => this.props.history.push(`/${this.props.projectName}/iam/roles/${role.name}`)}>
        ...
        { role ? (
          <InnerCard>
            <Pane padding={majorScale(4)}>
              ...
              <Heading paddingBottom={majorScale(4)} size={600}>Role / {role.name}</Heading>
              <TextInputField
                label="Name"
                onChange={this.handleUpdateName}
                value={this.state.name}
              ...
              />
              <Button marginTop={majorScale(2)} appearance="primary" disabled={this.state.unchanged} onClick={() => this.handleUpdate()}>Update Role</Button>
            </Pane>
            <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
              <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" onClick={() => this.setState({ showDeleteDialog: true })}>Delete Role...</Button>
            </Pane>
            <Pane>
              <Dialog
                isShown={this.state.showDeleteDialog}
              ...
              >
                You are about to delete the <strong>{this.props.roleName}</strong> role.
              </Dialog>
            </Pane>
          </InnerCard>
        ) : (
      ...
      <Pane width="50%">
        <InnerCard>
          ...
          >
            ...
            <Heading paddingBottom={majorScale(4)} size={600}>Create Role</Heading>
            <TextInputField
              label="Name"
              onChange={this.handleUpdateName}
              validationMessage={this.state.nameValidationMessage}
...
class Devices extends Component {
  ...
  renderDeviceOs(device) {
    var innerText = '-';
    if (device.info.hasOwnProperty('osRelease') && device.info.osRelease.hasOwnProperty('prettyName')) {
        ...
        <Pane width="70%">
          <InnerCard>
            ...
            >
              ...
              <Button margin={majorScale(2)} appearance="primary"
                onClick={() => this.props.history.push(`/${this.props.projectName}/devices/add`)}
              >
                Add Device
              </Button>
            </Pane>
            {this.state.devices && this.state.devices.length > 0 && (
              <Table>
                <Table.Head background="tint2">
                  <Table.TextHeaderCell flexBasis={90} flexShrink={0} flexGrow={0}>Status</Table.TextHeaderCell>
                  <Table.TextHeaderCell>Name</Table.TextHeaderCell>
                ...
                <Table.Body>
                  {this.state.devices.map(device => (
                    <Table.Row key={device.id} isSelectable onSelect={() => this.props.history.push(`/${this.props.projectName}/devices/${device.name}`)}>
                      <Table.TextCell flexBasis={90} flexShrink={0} flexGrow={0} alignItems="center" paddingRight="0">
                        {device.status === "offline" ? <Badge color="red">offline</Badge> : <Badge color="green">online</Badge>}
                      </Table.TextCell>
                      <Table.TextCell>{device.name}</Table.TextCell>
                      <Table.TextCell>{device.info.hasOwnProperty('ipAddress') ? device.info.ipAddress : ''}</Table.TextCell>
                      <Table.TextCell>{this.renderDeviceOs(device)}</Table.TextCell>
                    </Table.Row>
                  ))}
                </Table.Body>
              </Table>
...
class Device extends Component {
      ...
      <Tablist border="default">
        {this.state.tabs.map((tab, index) => (
          <Tab
            key={tab}
            id={tab}
            onSelect={() => this.props.history.push(`/${projectName}/devices/${deviceName}/${tab}`)}
            isSelected={index === selectedIndex}
          >
            {this.state.tabLabels[index]}
          </Tab>
  ...
  renderInner = (match) => {
    switch (match.params.appTab) {
      case 'overview':
        return <DeviceOverview projectName={match.params.projectName} device={this.state.device} history={this.props.history} />
      case 'ssh':
        return <DeviceSsh projectName={match.params.projectName} device={this.state.device} history={this.props.history} />
      case 'settings':
        return <DeviceSettings projectName={match.params.projectName} device={this.state.device} history={this.props.history} />
      default:
        return <Pane></Pane>
    }
  }
          ...
          {device ? (
            <React.Fragment>
              ...
              <InnerCard>
                ...
                <DeviceLabels projectName={this.props.projectName} device={device} />
              </InnerCard>
              <InnerCard>
                <Heading padding={majorScale(2)}>Services</Heading>
                <DeviceServices projectName={this.props.projectName} device={device} history={this.props.history} />
              </InnerCard>
            </React.Fragment>
          ) : (
            <CustomSpinner />
...
class DeviceServices extends Component {
  checkServices(applicationStatusInfo) {
    for (var i=0; i < applicationStatusInfo.length; i++) {
        if (applicationStatusInfo[i].serviceStatuses && applicationStatusInfo[i].serviceStatuses.length > 0){
          return true;
        }
    }
    return false;
              ...
              <Pane key={applicationInfo.application.id}>
                <Table.Body>
                  {applicationInfo.serviceStatuses.map((serviceStatus,index) => (
                    <Table.Row key={index}>
                      <Table.TextCell><Button color="#425A70" appearance="minimal" onClick={() => this.props.history.push(`/${this.props.projectName}/applications/${applicationInfo.application.name}`)}>{applicationInfo.application.name} / {serviceStatus.service}</Button></Table.TextCell>
          ...
          <React.Fragment key={deviceLabel.key}>
            <Table.Row>
              ...
              <Table.TextCell flexBasis={75} flexShrink={0} flexGrow={0}>
                <Pane display="flex">
                  <IconButton icon="edit" height={24} appearance="minimal" onClick={() => this.setEdit(i)} />
                  <IconButton icon="trash" height={24} appearance="minimal" onClick={() => this.setShowRemoveDialog(i)} />
                </Pane>
              </Table.TextCell>
            </Table.Row>
            <Pane>
      ...
      case 'edit':
        return (
          <Table.Row key={deviceLabel.key} height="auto">
            ...
            <Table.TextCell flexBasis={75} flexShrink={0} flexGrow={0}>
              <Pane display="flex">
                <IconButton icon="floppy-disk" height={24} appearance="minimal" onClick={() => this.setDeviceLabel(deviceLabel.key, deviceLabel.value, i)} />
                <IconButton icon="cross" height={24} appearance="minimal" onClick={() => this.cancelEdit(i)} />
              </Pane>
            </Table.TextCell>
          </Table.Row>
        );
        ...
        return (
          <Table.Row key={`new-${i}`} height="auto">
            ...
            <Table.TextCell flexBasis={75} flexShrink={0} flexGrow={0}>
              <Pane display="flex">
                <IconButton icon="floppy-disk" height={24} appearance="minimal" onClick={() => this.setDeviceLabel(deviceLabel.key, deviceLabel.value, i)} />
                <IconButton icon="cross" height={24} appearance="minimal" onClick={() => this.deleteDeviceLabel(deviceLabel.key, i)} />
              </Pane>
            </Table.TextCell>
          </Table.Row>
        );
    ...
    return (
      <Table>
        <Table.Head>
          ...
          <Table.TextHeaderCell flexBasis={75} flexShrink={0} flexGrow={0}></Table.TextHeaderCell>
        </Table.Head>
        <Table.Body>
          {this.state.labels.map((deviceLabel, i)  => this.renderDeviceLabel(deviceLabel, i))}
          <Table.Row key='add'>
            <Table.TextCell>
              <IconButton icon="plus" height={24} appearance="minimal" onClick={() => this.createNewDeviceLabel()} />
            </Table.TextCell>
            <Table.TextCell></Table.TextCell>
            <Table.TextCell flexBasis={75} flexShrink={0} flexGrow={0}></Table.TextCell>
          </Table.Row>
        </Table.Body>
      </Table>
    );
        ...
        <InnerCard>
          ...
          >
            ...
            <Table paddingBottom={majorScale(2)} >
              <Table.Row>
                <Table.Cell paddingLeft="0"><Heading size={600}>Device Settings</Heading></Table.Cell>
                    <Table.Cell flexBasis={90} flexShrink={0} flexGrow={0}>{device.status === "offline" ? <Badge color="red">offline</Badge> : <Badge color="green">online</Badge>}</Table.Cell>
              </Table.Row>
            </Table>
            <Text><strong>ID: </strong>{device.id}</Text>
            <TextInputField
              label="Name"
              onChange={this.handleUpdateName}
              value={this.state.name}
            ...
            <Pane marginTop={majorScale(2)}>
              <Button appearance="primary" disabled={this.state.unchanged} onClick={this.handleUpdate}>Update Settings</Button>
            </Pane>
          </Pane>
          <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
            <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" onClick={() => this.setState({ showRemoveDialog: true })}>Remove Device...</Button>
          </Pane>
          <Pane>
            <Dialog
              isShown={this.state.showRemoveDialog}
          ...
          <InnerCard>
            ...
            >
              <Pane
                ...
                paddingBottom={majorScale(2)}
              >
                <BackButton onClick={() => this.props.history.push(`/${this.props.projectName}/devices`)}>Devices</BackButton>
                <Button appearance="primary" onClick={() => this.handleAddNewDevice()}>Add Another Device</Button>
              </Pane>
              <Pane
                display="flex"
                flexDirection="row"
              ...
              >
                ...
                <Text>Device registration ID <Code fontFamily="mono" background="#234361" color="white">{this.state.deviceRegistrationToken.id}</Code> created.</Text>
              </Pane>
              <Card
                display="flex"
                flexDirection="column"
        ...
        <Pane width="70%">
          <InnerCard>
            ...
            >
              ...
              <Button margin={majorScale(2)} appearance="primary"
                onClick={() => this.props.history.push(`/${this.props.projectName}/applications/create`)}
              >
                Create Application
              </Button>
            </Pane>
              ...
              <Table>
                ...
                <Table.Body>
                  {this.state.applications.map(application => (
                    <Table.Row key={application.id} isSelectable onSelect={() => this.props.history.push(`/${this.props.projectName}/applications/${application.name}`)}>
                      <Table.TextCell>{application.name}</Table.TextCell>
                      <Table.TextCell>{application.latestRelease ? moment(application.latestRelease.createdAt).fromNow() : '-'}</Table.TextCell>
                      <Table.TextCell>{application.deviceCounts.allCount}</Table.TextCell>
                    </Table.Row>
                  ))}
                </Table.Body>
              </Table>
...
class Application extends Component {
  ...
  renderTablist = (match) => {
    ...
    switch (match.params.appTab) {
      ...
      case 'settings':
        ...
        break;
      default:
        this.props.history.push(`/${projectName}/applicationName/${applicationName}`);
    }
    return (
      <Tablist border="default">
        {this.state.tabs.map((tab, index) => (
          <Tab
            key={tab}
            id={tab}
            onSelect={() => this.props.history.push(`/${projectName}/applications/${applicationName}/${tab}`)}
            isSelected={index === selectedIndex}
          >
            {this.state.tabLabels[index]}
          </Tab>
  ...
  renderInner = (match) => {
    switch (match.params.appTab) {
      case 'overview':
        return <ApplicationOverview projectName={match.params.projectName} application={this.state.application} history={this.props.history}/>
      case 'scheduling':
        return <ApplicationScheduling projectName={match.params.projectName} application={this.state.application} history={this.props.history}/>
      case 'settings':
        return <ApplicationSettings projectName={match.params.projectName} application={this.state.application} history={this.props.history}/>
      default:
        return <Pane></Pane>
    }
  }
...
class ApplicationOverview extends Component {
  render() {
    const currentConfig = this.props.application.latestRelease ? this.props.application.latestRelease.config : '';
    return (
      <Pane width="70%" paddingBottom={majorScale(4)}>
        <Heading paddingTop={majorScale(4)} paddingBottom={majorScale(1)} size={600}>{this.props.application.name}</Heading>
        <InnerCard>
          <Heading paddingTop={majorScale(2)} paddingLeft={majorScale(2)}>Scheduling Rule</Heading>
          <Card
            display="flex"
            flexDirection="column"
            alignItems="left"
        ...
        <InnerCard>
          <Heading paddingTop={majorScale(2)} paddingLeft={majorScale(2)}>Current Config</Heading>
          <Card
            display="flex"
            flexDirection="column"
            alignItems="center"
        ...
        <InnerCard>
          ...
          >
            ...
            <Button
              ...
              appearance="primary"
              onClick={() => this.props.history.push(`/${this.props.projectName}/applications/${this.props.application.name}/deploy`)}
            >
              Create New Release
            </Button>
          </Pane>
          <Releases projectName={this.props.projectName} applicationName={this.props.application.name} history={this.props.history}/>
        </InnerCard>
      </Pane>
    );
  }
      ...
      <Pane width="50%">
        <InnerCard>
          ...
          >
            ...
            <Heading paddingBottom={majorScale(4)} size={600}>Scheduling</Heading>
            <Label
              htmlFor="description-textarea"
              marginBottom="4"
              display="block"
      ...
      <Pane width="50%">
        <InnerCard>
          ...
          >
            ...
            <Heading paddingBottom={majorScale(4)} size={600}>Application Settings</Heading>
            <TextInputField
            ...
            <Textarea
              ...
              value={this.state.description}
            />
            <Button marginTop={majorScale(2)} appearance="primary" disabled={this.state.unchanged} onClick={this.handleUpdate}>Update Settings</Button>
          </Pane>
          <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
            <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" onClick={() => this.setState({ showDeleteDialog: true })}>Delete Application...</Button>
          </Pane>
          <Pane>
            <Dialog
              isShown={this.state.showDeleteDialog}
            ...
            >
              You are about to delete the <strong>{this.props.application.name}</strong> application.
              </Dialog>
          </Pane>
        </InnerCard>
      </Pane>
  ...
  getReleasedBy = (release) => {
    if (release) {
      if (release.createdByUser) {
        ...
        return (
          <Link color="neutral" href={memberUrl}>{release.createdByUser.firstName} {release.createdByUser.lastName}</Link>
        )
      } else if (release.createdByServiceAccount) {
        const serviceAccountUrl = '../../iam/serviceaccounts/' + release.createdByServiceAccount.name
        return (
          <Link color="neutral" href={serviceAccountUrl}>{release.createdByServiceAccount.name}</Link>
        )
      }
    }
    return "-"
    ...
    return (
      <React.Fragment>
        ...
        {this.state.releases && this.state.releases.length > 0 && (
          <Table>
            <Table.Head>
              <Table.TextHeaderCell flexGrow={3} flexShrink={3}>Release</Table.TextHeaderCell>
              <Table.TextHeaderCell flexGrow={2} flexShrink={2}>Released By</Table.TextHeaderCell>
              <Table.TextHeaderCell>Started</Table.TextHeaderCell>
              <Table.TextHeaderCell>Device Count</Table.TextHeaderCell>
            </Table.Head>
            <Table.Body>
              {this.state.releases.map(release => (
                <Table.Row key={release.id} isSelectable onSelect={() => this.showSelectedRelease(release)}>
                  <Table.TextCell flexGrow={3} flexShrink={3}>{release.id}</Table.TextCell>
                  <Table.TextCell flexGrow={2} flexShrink={2}>{this.getReleasedBy(release)}</Table.TextCell>
                  <Table.TextCell>{moment(release.createdAt).fromNow()}</Table.TextCell>
      ...
      >
        <Release release={this.state.selectedRelease} projectName={this.props.projectName} applicationName={this.props.applicationName} history={this.props.history}></Release>
      </SideSheet>
      </React.Fragment >
    );
  }
      ...
      <React.Fragment>
        ...
        >
          ...
          <Pane
            ...
            marginBottom={majorScale(2)}
          >
            <Label><strong>Released By:</strong> {this.getReleasedBy(release)}</Label>
            <Label><strong>Started:</strong> {moment(release.createdAt).fromNow()}</Label>
          </Pane>
          <Pane
            display="flex"
            justifyContent="space-between"
          ...
          >
            ...
            <Button appearance="primary" justifyContent="center" onClick={() => this.setState({ showConfirmDialog: true })}>Revert to this Release</Button>
          </Pane>
          <Editor
            width="100%"
            height="300px"
            value={release.config}
            readOnly
          />
        </Pane>
        <Dialog
        ...
        >
          This will create a new release to application <strong>{this.props.applicationName}</strong> using the config from release <strong>{release.id}</strong>.
        </Dialog>
      </React.Fragment>
    )
  }
    ...
    return (
      ...
      >
        ...
        { this.state.submitError && (
          <Alert marginBottom={majorScale(2)} paddingTop={majorScale(2)} paddingBottom={majorScale(2)} intent="warning" title={this.state.submitError} />
        )}
        <Pane is="form">
          <TextInputField
            label="Email"
        ...
        >
          ...
          <Link paddingLeft={majorScale(1)} href='/forgot'>Reset your password</Link>
        </Pane>
        <Pane
          display="flex"
          flexDirection="row"
            ...
            <Table>
              ...
              <Table.Body>
                {this.state.membershipsFull.map(membershipFull => (
                  <Table.Row key={membershipFull.project.id} isSelectable onSelect={() => this.props.history.push(`/${membershipFull.project.name}`)}>
                    <Table.TextCell>{membershipFull.project.name}</Table.TextCell>
                    <Table.TextCell>{membershipFull.project.deviceCounts.allCount}</Table.TextCell>
                    <Table.TextCell>{membershipFull.project.applicationCounts.allCount}</Table.TextCell>
                  </Table.Row>
                ))}
              </Table.Body>
            </Table>
...
class ProjectSettings extends Component {
  ...
  render() {
    const projectName = this.props.projectName;
    const heading = "Project / " + projectName;
    return (
      <React.Fragment>
        <TopHeader user={this.props.user} heading={heading} history={this.props.history} />
        <Pane width="50%">
          <InnerCard>
            ...
            >
              ...
              <Heading paddingBottom={majorScale(4)} size={600}>Project Settings</Heading>
              <TextInputField
                label="Project Name"
                onChange={this.handleUpdateName}
                value={this.state.name}
            ...
            <Pane borderTop="default" marginRight={majorScale(4)} marginLeft={majorScale(4)} marginBottom={majorScale(4)}>
              <Button marginTop={majorScale(4)} iconBefore="trash" intent="danger" onClick={() => this.setState({ showDeleteDialog: true })}>Delete Project...</Button>
            </Pane>
            <Pane>
              <Dialog
                isShown={this.state.showDeleteDialog}
                ...
                isConfirmDisabled={this.state.disableDeleteConfirm}
              >
                This action <strong>cannot</strong> be undone. This will permanently delete the <strong>{this.props.projectName}</strong> project.<p></p>Please type in the name of the project to confirm.
                <TextInput
                  marginTop={majorScale(1)}
          ...
          {this.state.tabs.map((tab, index) => (
            ...
            >
              ...
              >
                ...
                <Text paddingTop={majorScale(1)}>{this.state.tabLabels[index]}</Text>
              </Pane>
            </SidebarTab>
          ))}
        </TabNavigation>
          ...
          {this.state.footerTabs.map((tab, index) => (
            ...
            >
              ...
              >
                ...
                <Text paddingTop={majorScale(1)}>{this.state.footerTabLabels[index]}</Text>
              </Pane>
            </SidebarTab>
          ))}
        </Pane>
        ...
        return (
          <Switch>
            <Route
              path={`${match.path}/add`}
              render={(route) => (
                <AddDevice user={user} projectName={projectName} history={this.props.history} />
              )}
            />
            <Route
              exact
              ...
              render={(route) => (
                <Redirect to={`${route.match.url}/overview`} />
              )}
            />
            <Route
              path={`${match.path}/:deviceName/:appTab`}
              render={(route) => (
                <Device user={user} projectName={projectName} deviceName={route.match.params.deviceName} deviceRoute={route.match} history={this.props.history} />
              )}
            />
            <Route
              exact
              path={match.path}
              render={(route) => (
                <Devices user={user} projectName={projectName} history={this.props.history} />
              )}
            />
          </Switch>
        )
        ...
        return (
          <Switch>
            <Route
              path={`${match.path}/create`}
              render={(route) => (
                <CreateApplication user={user} projectName={projectName} history={this.props.history} />
              )}
            />
            <Route
              path={`${match.path}/:applicationName/deploy`}
              render={(route) => (
                <CreateRelease user={user} projectName={projectName} applicationName={route.match.params.applicationName} history={this.props.history} />
              )}
            />
            <Route
              exact
            ...
            <Route
              ...
              render={(route) => (
                <Application user={user} projectName={projectName} applicationName={route.match.params.applicationName} applicationRoute={route.match} history={this.props.history} />
              )}
            />
            <Route
              exact
              path={match.path}
              render={(route) => (
                <Applications user={user} projectName={projectName} history={this.props.history} />
              )}
            />
          </Switch>
        )
        ...
        return (
          <Switch>
            <Route
              ...
              render={(route) => (
                <Iam user={user} projectName={projectName} match={route.match} history={this.props.history} />
            )}/>
            <Route
              render={(route) => (
                <Redirect to={`${route.match.url}/members`} />
            )}/>
          </Switch>
        )
      case 'settings':
        return (
          <Switch>
            <Route
              ...
              path={match.path}
              render={(route) => (
                <ProjectSettings user={user} projectName={projectName} history={this.props.history} />
              )}
            />
          </Switch>
        )
    ...
    return (
      ...
      >
        ...
        {this.state.passwordRecoveryError && (
          <Alert marginBottom={majorScale(2)} paddingTop={majorScale(2)} paddingBottom={majorScale(2)} intent="warning" title={this.state.passwordRecoveryError} />
        )}
        {this.state.invalidToken ? (
          <React.Fragment>
            <Alert marginBottom={majorScale(2)} paddingTop={majorScale(2)} paddingBottom={majorScale(2)} intent="warning"
    ...
    return (
      ...
      >
        <Switch>
          ...
          <Route exact path="/projects" render={route => (
            <ProjectSelector user={user} match={route.match} history={route.history} />
          )} />
          <Route exact path="/projects/create" render={route => (
            <CreateProject user={user} match={route.match} history={route.history} />
          )} />
          <Route exact path="/:projectName" render={(route) => (
            <Redirect to={`${route.match.url}/devices`} />
          )} />
          <Route path="/:projectName/:tab" render={(route) => (
            <InnerOogie user={user} match={route.match} history={route.history} />
          )} />
        </Switch>
      </Pane>
    );
      ...
      <Pane>
        { this.state.authenticationCheckCompleted ? (
          <Switch>
            ...
            <Route path="/confirm/:token" render={(route) => <Confirm token={route.match.params.token} />} />
            <Route path="/recover/:token" render={(route) => <PasswordRecovery token={route.match.params.token} history={route.history} />} />
          </Switch>
        ) : (
          <CustomSpinner />
        )}
